...
deathAgeCounter = 0
deathStarvationCounter = 0
deathEscapeCounter = 0
...
deathsThisTurn = deathSquishCounter + deathAgeCounter + deathStarvationCounter
deathEscapesThisTurn = deathEscapeCounter
pushedCellCounter = 0
aliveCount = 0
...
class Cell:
    def __init__(self, x, y, id, organism=None):
        ...
        self.attractiveness = CELL_BASE_ATTRACTIVENESS
        self.genome = {
    ...
    def reproduce(self, grid, lightGrid, counter):
        ...
        if random.random() < CELL_FERTILITY_CHANCE or self.attractiveness > 9 or self.energy > CELL_FERTILITY_ENERGY:
            ...
            if grid[x, y] == 0 or grid[x, y] is None:  # Empty spot
                ...
                baby_cell.speed = max(0.5, min(2.0, cell.speed + random.uniform(CELL_BABY_MUTATION_SPEED_MIN, CELL_BABY_MUTATION_SPEED_MIX)))
    ...
    def decay(self, lightGrid, inertGrid):
        ...
        if self.energy <= 0 or self.age > (self.resilience * random.uniform(CELL_DEATH_AGE_MIN, CELL_DEATH_AGE_MAX)):  # Death by starvation or old age
            ...
            deathAgeCounter += 1
            deathStarvationCounter += 1
            self.energy = 0
            self.state = "inert"
            lightGrid[self.x, self.y] += CELL_DEATH_LIGHT_RELEASE  # Dead cells release light for some reason
            inertGrid[self.x, self.y] += CELL_DEATH_INERT_RELEASE # Drop inert resources onto inert grid
...
def enrich_environment(lightGrid, waifuGrid, inertGrid):
    ...
    for _ in range(ENVIRONMENT_LIGHT_ENRICHMENT_SOURCE_NUM):  # Number of light sources
        ...
        lightGrid[x, y] = min(lightGrid[x, y] + ENVIRONMENT_LIGHT_ENRICHMENT, ENVIRONMENT_LIGHT_MAX_CLIP)  # Cap at max brightness
    lightGrid -= ENVIRONMENT_GRID_DECAY_RATE
    lightGrid = np.clip(lightGrid, ENVIRONMENT_LIGHT_MIN_CLIP, ENVIRONMENT_LIGHT_MAX_CLIP)
...
for step in range(NUM_STEPS):
    ...
    ax.set_title(f"Step {step + 1} ({aliveCount})")
    print(f"Done plotting, {aliveCount} cells are alive and displayed. Starting env changes")
    plt.pause(0.1)
        ...
        for y in range(grid.shape[1]):
            ...
            if isinstance(cell, Cell):
                if cell.step_count < step:
                    ...
                    cell.reproduce(grid, lightGrid, counter)
...
print(f"Turn Summary: There are currently {aliveCount} living cells. There were {babysThisTurn} babies born, {deathsThisTurn} cells died, and {deathEscapesThisTurn} cells evaded death! ")
